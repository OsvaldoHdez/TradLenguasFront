...
import { Paper, TextField, ImageListItem, ImageList, Box, CssBaseline, Button, Grid, Typography } from "@mui/material";
import { styled } from '@mui/material/styles';
import { Signimage } from "./images/ASL"
import { SignimageKey } from "./images/ASL/keyboard"
...
const TraductorLS = () => {
    ...
    return (
        <Box sx={{ marginLeft: 0 }}>
          <CssBaseline />
            <Box>
              ...
              <Box sx={{display: 'flex', justifyContent: 'center', marginLeft: 2 }}>
                  <ImageList sx={{ width: 1000, height: 150 }} cols={9} rowHeight={140}>
                      {itemData1.map((item) => (
                      <ImageListItem key={item.img}>
                          <img
                              src={`${item.img}?w=164&h=164&fit=crop&auto=format`}
          ...
          <Grid sx={{ display: 'flex', justifyContent: "left", marginTop: -47, marginLeft: -21}}>
            <Item>
